...
```bash
wget https://github.com/nasa/GeneLab_AmpliconSeq_Workflow/releases/download/v1.0.6/NF_AmpIllumina_1.0.6.zip
unzip NF_AmpIllumina_1.0.6.zip && cd NF_AmpIllumina_1.0.6
```
...
> ***Note:** All the commands in this step assume that the workflow will be run from within the `NF_AmpIllumina_1.0.6` directory that was downloaded in [step 2](#2-download-the-workflow-files) above. They may also be run from a different location by providing the full path to the main.nf workflow file in the `NF_AmpIllumina_1.0.6` directory.*
...
* `main.nf` - Instructs Nextflow to run the NF_AmpIllumina workflow. If running in a directory other than `NF_AmpIllumina_1.0.6`, replace with the full path to the NF_AmpIllumina main.nf workflow file.
